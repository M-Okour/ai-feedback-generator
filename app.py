...
def extract_student_signatures_from_excel(classlist_bytes):
    """
    Extract embedded student signature images from Excel column:
    ...
    for cell in ws[header_row]:
        ...
        header_norm = header.lower().replace(" ", "_")
...
Write feedback in this exact style:
{pc}: {level}:
[Feedback sentence]

Rules:
- Do not repeat the PC and level inside the feedback sentence.
- Use the student's first name once at the beginning of the feedback sentence.
- Write 1 to 2 sentences only.
- Sound natural, human, supportive, and professional.
...
"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Remove heading if AI includes it, because heading is added separately in Word.
            text = re.sub(rf"^{re.escape(pc)}\s*:\s*{re.escape(level)}\s*:\s*", "", text).strip()

            return text

        except RateLimitError:
            wait_time = 8 * (attempt + 1)
            st.warning(f"Rate limit reached for {pc}. Retrying in {wait_time} seconds...")
...
def fill_name_and_id_in_table(table, student_name, student_id):
    for row in table.rows:
        fill_adjacent_or_empty(row, ["Student Name"], student_name)
        fill_adjacent_or_empty(row, ["ID No.", "ID No", "Student ID"], student_id)
...
def insert_signature_image(cell, signature_bytes, width_inches=1.2):
    insert_image_in_cell(cell, signature_bytes, width_inches)
...
def find_grade_column_indices(table):
    ...
    return {
        "Not Yet Competent": 2,
        "Competent": 3,
        "Competent with Merit": 4,
        "Competent with Distinction": 5
    }
...
def fill_summative_grade_in_table(table, pc_marks):
    ...
    for row in table.rows:
        ...
        if "Summative Assessment Grade %:" in text:
            fill_adjacent_or_empty(row, ["Summative Assessment Grade %:"], int(summative))
            return
...
def build_summative_comment(student_name, overall_level, sa_number, feedback_rows, sa2_date, lo_data):
    ...
    if failed_pcs:
        ...
        if sa2_date.strip():
            summary_comment += f" The second attempt will be conducted on {sa2_date}."
    ...
    lo_comments = build_lo_comments(feedback_rows, lo_data)
...
def add_compact_paragraph(cell, text="", bold_prefix=None, font_size=9, space_after=0):
...
def build_feedback_content_in_cell(cell, feedback_rows, student_name, overall_level, sa_number, sa2_date, lo_data):
    cell.text = ""
    ...
    for lo_comment in comment_data["lo_comments"]:
        ...
        if match:
            ...
            add_compact_paragraph(cell, text=lo_comment, font_size=9, space_after=0)

    add_compact_paragraph(cell, text="", font_size=9, space_after=0)

    add_compact_paragraph(
        cell,
        text=comment_data["summary_comment"],
        font_size=10,
        space_after=0
    )


def insert_feedback_at_assessor_feedback(doc, feedback_rows, student_name, overall_level, sa_number, sa2_date, lo_data):
        ...
        for row in table.rows:
            ...
            for i, cell in enumerate(cells):
                if "Assessor Feedback:" in cell.text or "Assessor Feedback" in cell.text:
                    ...
                    build_feedback_content_in_cell(
                        target_cell,
                        feedback_rows,
                        student_name,
                        overall_level,
...
def fill_template(
    ...
    doc = insert_feedback_at_assessor_feedback(
        doc=doc,
        feedback_rows=feedback_rows,
        student_name=student_name,
        overall_level=overall_level,
